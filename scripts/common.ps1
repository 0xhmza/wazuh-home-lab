...
function Write-WazuhDashboardOverride {
    ...
    # NOTE: Compose v2 merges list fields like 'ports' additively across files.
    # Without '!override', the base 443:5601 mapping survives and you end up
    # with the dashboard bound to BOTH ports - which fails the second time the
    # script runs (since 443 still needs admin / is often busy). '!override'
    # forces a complete replacement of the list.
    $content = @"
# Auto-generated by Wazuh Home Lab - do not edit by hand.
# Replaces the dashboard host port (443) with a free, non-privileged port.
services:
  wazuh.dashboard:
    ports: !override
      - "$Port`:5601"
